...
class SudokuTable:
    ...
    def validatePos(self, pos: tuple, point: int) -> bool:
        ...
        for i in range(boxRow, boxRow + self._boxHeight):
            for j in range(boxCol, boxCol + self._boxWidth):
                # Ignore the row and columns that were already checked above
                if self._table[i][j] == point and i != row and j != col:
                    return False
    ...
    def prettyPrint(self) -> None:
        numVertLines = max(0, (self._width // self._boxWidth) - 1)

        for i in range(len(self._table)):
            if i % self._boxHeight == 0 and i != 0:
                print("- " * (self._width + numVertLines))
